...
\begin{itemize}[nosep]
  ...
  \item A 1\,kHz tone (200\,ms) is played when a timed rest ends, i.e.\ at the start of
        \textsc{Plan} and at the start of \textsc{Break}, so the subject does not have to
        watch the screen during rest. A double tone marks the end of the session.
...
Table~\ref{tab:blocks}; trials are separated by a break of free length during which the subject
scrambles the cube using the scramble printed by the script; the next trial is
started manually with ENTER and the break duration is logged. A session consists
...
\textsc{Break}       & free (logged) & 50 & Relax, then scramble the cube with the printed scramble. Put it down and press ENTER to start the next trial. The break length is not fixed but is recorded. \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tikzpicture}[x=0.9cm, every node/.style={font=\small}]
  \foreach \x/\w/\name/\col in {0/2/Rest 20 s/gray!30, 2/2/Plan/blue!25, 4/3/Solve/red!25, 7/3/Rest 45 s/gray!30, 10/4/Break (free; scramble)/green!20} {
