...
experiment_name = "BaselineTwoHandSimulation".replace(" ", "_")
...
# Block Design
blocks = [ "Baseline Start" ]
durations = [ 20 ] # Seconds
block_order = [ 0] # NOTE : This are indices into the Blocks array
wait_for_input_blocks = [ False ] # If True : to proceed to next block, a manual input is needed
...
if not blockless:
    ready = input(f"Press [ ENTER ] to start block design | First block is [ {blocks[block_order[0]]} ]")
