...
experiment_name = "PostCVitaminDanielTwoHand".replace(" ", "_")
...
# Block Design
blocks = [ "Rest", "Left","Right"]
durations = [ 15, 10, 10 ] # Seconds
block_order = [0, 1, 0, 2, 
               0, 1, 0, 2, 
               0, 1, 0, 2, 
               0, 1, 0, 2, 
               0, 1, 0, 2, 
               0, 1, 0, 2, 
               0] # NOTE : This are indices into the Blocks array
wait_for_input_blocks = [ False, False, False ] # If True : to proceed to next block, a manual input is needed

# Baseline Configuration
baseline_duration = 20 # 30s pre and post baselines
use_baseline = False
...
use_ur3     = False # NOTE : This starts a seperate program thread handling the connection
...
UR_SERVER_HOST = '192.168.0.101'  # Listen on all interfaces
    ...
    try:
        while True:
            ...
            try:
                ...
                logging.info(f"UR Server : Received message from UR3 : {message}") # Only put this into the log file, dont print to console

                if message in markers:
                    push_marker(markers[message])
                else:
                    logging.info(f"Unknown message from controller: {message}")
...
if not blockless:
    ready = input(f"Press [ ENTER ] to start block design { blocks[block_order[0]]}")
    run_block_design()
